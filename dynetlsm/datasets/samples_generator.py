...
__all__ = ['network_from_dynamic_latent_space',
           ...
           'synthetic_time_homogeneous_dynamic_network',
           'synthetic_clusterfree_dynamic_network']
...
def synthetic_time_homogeneous_dynamic_network(n_nodes=120, n_time_steps=9, n_groups=6,
                                              intercept=1.0, lmbda=0.8, sticky_const=20.,
                                              sigma_shape=6, sigma_scale=20, nu=4, is_directed=False,
                                              is_weighted=False, random_state=42):
    rng = check_random_state(random_state)
...
def synthetic_clusterfree_dynamic_network(n_nodes=120, n_time_steps=9, n_groups=10,
                                          intercept=1.0, sigma=0.000001, nu=4, is_directed=False,
                                          is_weighted=False, random_state=42):
    rng = check_random_state(random_state)

    mean = np.repeat(0, 2)
    cov = 0.000018 * np.identity(2)
    mus = rng.multivariate_normal(mean, cov, n_groups)
    var = 0.000002
    w0 = np.repeat(1/n_groups, n_groups)
    ...
    for group_id in range(n_groups):
        group_size = np.sum(z0 == group_id)
        X0[z0 == group_id, :] = (var * rng.randn(group_size, 2) +
                                 mus[group_id])
    X.append(X0)

    for t in range(1, n_time_steps):
        Xt = np.zeros((n_nodes, 2), dtype=np.float64)
        Xt = sigma * rng.randn(n_nodes, 2) + X[t - 1]
    ...
    return Y, X, intercept, radii, nu, probas, Y_star